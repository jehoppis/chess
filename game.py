...
class Game:
    ...
    def __init__(self):
        self.turn_count = 1  # Tracks current turn count. Updated by move_piece function.
        ...
        self.squares = dict()  # This dictionary is the meat of the class. It will contain info about what piece is on
        # a given square, who the piece belongs to, what diagonals the square belongs to, etc.

        self.diagonalsTLBR = dict({k: [] for k in range(15)})  # Create diagonals slanting from top
        # left to bottom k=0 corresponds to a1. Diagonals are used by is_legal to determine legality of moves for
        # bishops and queens.

        self.diagonalsBLTR = dict({n: [] for n in range(15)})  # Create diagonals slanting from bottom left to top right
        #  k=0 corresponds to a8.

        self.black_king = ('e', 8)  # Property to record the location of black king. Used by is_threat to determine if
        # black is in check.
        self.white_king = ('e', 1)

        self.history = dict()  # Records all moves

        for i, row in enumerate(self.rows):  # Filling in the squares dictionary with default info.
            for j, column in enumerate(self.columns):
                self.diagonalsTLBR[i + j].append((column, row))
                self.diagonalsBLTR[7 - i + j].append((column, row))
                if ((i + j) % 2) == 0:
                    self.squares.update({(column, row): dict(occupied=False, player=False, TLBR=i + j, BLTR=7 - i + j)})
                if ((i + j) % 2) == 1:
                    self.squares.update({(column, row): dict(occupied=False, player=False, TLBR=i + j, BLTR=7 - i + j)})
        ...
        for i in [4, 5]:
            for j in self.columns:
                # noinspection PyTypeChecker
                self.squares[(j, i)]['en passant'] = [0, '-']  # This is a flag for the En Passant move. The default
    ...
    def visualize(self):
        def abbreviate(piece):
            if piece is False:
                ...
                return 'K'
        table = [[((self.squares[self.columns[j], i]['player'] == 'black') * 'B-'
                 + (self.squares[self.columns[j], i]['player'] == 'white') * 'W-'
                 + ((i + j) % 2)*(self.squares[self.columns[j], i]['player'] is False) * '--'
                 + ((i + j - 1) % 2)*(self.squares[self.columns[j], i]['player'] is False) * '----'
                 + abbreviate(self.squares[self.columns[j], i]['occupied']))
                 for j in range(len(self.columns))] for i in range(8, 0, -1)]
    ...
    def is_legal(self, current_pos, end_pos, color):
        ...
        # Pawn----------------------------------------------------------------------------------------------------------
            ...
            if current_pos[0] == end_pos[0]:
                if self.squares[(current_pos[0], int(current_pos[1]))]['player'] == 'white':
                    if int(current_pos[1]) == 2:
                        if self.squares[(current_pos[0], 3)]['occupied'] is False and \
                                self.squares[(current_pos[0], 4)]['occupied'] is False and \
                                int(end_pos[1]) == 4:
                            return True
                    if int(current_pos[1]) + 1 == int(end_pos[1]) and \
                            self.squares[(current_pos[0], int(end_pos[1]))]['occupied'] is False:
                        return True
                    else:
                        return False
                elif self.squares[(current_pos[0], int(current_pos[1]))]['player'] == 'black':
                    if int(current_pos[1]) == 7:
                        if self.squares[(current_pos[0], 6)]['occupied'] is False and \
                                self.squares[(current_pos[0], 5)]['occupied'] is False and \
                                int(end_pos[1]) == 5:
                            return True
                    if int(current_pos[1]) - 1 == int(end_pos[1]) and \
                            self.squares[(current_pos[0], int(end_pos[1]))]['occupied'] is False:
            ...
            if current_pos[0] != end_pos[0]:
                if self.squares[(current_pos[0], int(current_pos[1]))]['player'] == 'white':
                    if cur_col_index + 1 == end_col_index and int(current_pos[1]) + 1 == int(end_pos[1]) and \
                            self.squares[(end_pos[0], int(end_pos[1]))]['occupied'] is not False:
                        return True
                    elif cur_col_index - 1 == end_col_index and int(current_pos[1]) + 1 == int(end_pos[1]) and \
                            self.squares[(end_pos[0], int(end_pos[1]))]['occupied'] is not False:
                        return True
                    elif cur_col_index + 1 == end_col_index and int(current_pos[1]) + 1 == int(end_pos[1]) and \
                            int(current_pos[1]) == 5:
                        if self.squares[(current_pos[0], int(current_pos[1]))]['en passant'] == \
                ...
                if current_pos[0] == end_pos[0]:
                    if int(current_pos[1]) < int(end_pos[1]):
                        for i in range(int(current_pos[1]) + 1, int(end_pos[1])):
                            if self.squares[(current_pos[0], i)]['occupied'] is not False:
                                spaces_are_empty = False
                    elif int(current_pos[1]) > int(end_pos[1]):
                        for i in range(int(current_pos[1]) - 1, int(end_pos[1]), -1):
                            if self.squares[(current_pos[0], i)]['occupied'] is not False:
                                spaces_are_empty = False
                    if spaces_are_empty is True and self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                        return True
                    elif not (spaces_are_empty is True and
                    ...
                    if cur_col_index < end_col_index:
                        for i in range(cur_col_index + 1, end_col_index):
                            if self.squares[(self.columns[i], int(current_pos[1]))]['occupied'] is not False:
                                spaces_are_empty = False
                    ...
                    if spaces_are_empty is True and self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                        ...
                        return False
        # Knight--------------------------------------------------------------------------------------------------------
        elif self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'knight':
            if ((int(current_pos[1]) + 2 == int(end_pos[1]) and (cur_col_index + 1 == end_col_index or
                                                                 cur_col_index - 1 == end_col_index))
                or (int(current_pos[1]) - 2 == int(end_pos[1]) and (cur_col_index + 1 == end_col_index or
                                                                    cur_col_index - 1 == end_col_index))
                or (cur_col_index - 2 == end_col_index and (int(current_pos[1]) + 1 == int(end_pos[1]) or
                                                            int(current_pos[1]) - 1 == int(end_pos[1])))
                or (cur_col_index + 2 == end_col_index and (int(current_pos[1]) + 1 == int(end_pos[1]) or
                                                            int(current_pos[1]) - 1 == int(end_pos[1])))) and \
                    self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                return True
            else:
                return False
        # Bishop--------------------------------------------------------------------------------------------------------
        elif self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'bishop':
            spaces_are_empty = True
            if self.squares[(current_pos[0], int(current_pos[1]))]['TLBR'] == \
                    self.squares[(end_pos[0], int(end_pos[1]))]['TLBR']:
                cur_diag_index = \
                    self.diagonalsTLBR[self.squares[(current_pos[0], int(current_pos[1]))]['TLBR']]\
                        .index((current_pos[0], int(current_pos[1])))
                end_diag_index = \
                    self.diagonalsTLBR[self.squares[(end_pos[0], int(end_pos[1]))]['TLBR']]\
                        .index((end_pos[0], int(end_pos[1])))
                if cur_diag_index < end_diag_index:
                    for i in range(cur_diag_index + 1, end_diag_index):
                        if self.squares[self.diagonalsTLBR[self.squares[(current_pos[0],
                                                                         int(current_pos[1]))]['TLBR']][i]]['occupied']\
                                is not False:
                            spaces_are_empty = False
                    if spaces_are_empty and self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                        return True
                    else:
                        return False
                    ...
                    if spaces_are_empty and self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                        ...
                        return False
            elif self.squares[(current_pos[0], int(current_pos[1]))]['BLTR'] == \
                    self.squares[(end_pos[0], int(end_pos[1]))]['BLTR']:
                cur_diag_index = \
                    self.diagonalsBLTR[self.squares[(current_pos[0], int(current_pos[1]))]['BLTR']] \
                        .index((current_pos[0], int(current_pos[1])))
                end_diag_index = \
                    self.diagonalsBLTR[self.squares[(end_pos[0], int(end_pos[1]))]['BLTR']] \
                        .index((end_pos[0], int(end_pos[1])))
                if cur_diag_index < end_diag_index:
                    for i in range(cur_diag_index + 1, end_diag_index):
                        if self.squares[self.diagonalsBLTR[self.squares[(current_pos[0],
                                                                        int(current_pos[1]))]['BLTR']][i]]['occupied'] \
                                is not False:
                            spaces_are_empty = False
                    if spaces_are_empty and self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                        return True
        ...
        # Queen---------------------------------------------------------------------------------------------------------
            ...
            elif self.squares[(current_pos[0], int(current_pos[1]))]['TLBR'] == \
                    self.squares[(end_pos[0], int(end_pos[1]))]['TLBR']:
                cur_diag_index = \
                    self.diagonalsTLBR[self.squares[(current_pos[0], int(current_pos[1]))]['TLBR']]\
                        .index((current_pos[0], int(current_pos[1])))
        ...
        # King----------------------------------------------------------------------------------------------------------
            ...
            if end_col_index in [cur_col_index - 1, cur_col_index, cur_col_index + 1] and \
                    int(end_pos[1]) in [int(current_pos[1]) - 1, int(current_pos[1]), int(current_pos[1]) + 1] and \
                    self.squares[(end_pos[0], int(end_pos[1]))]['player'] != color:
                return True
            else:
                return False
    ...
    def is_threat(self, current_pos, end_pos, color):
        if self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'king' and color == 'white':
            self.white_king = (end_pos[0], int(end_pos[1]))
        elif self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'king' and color == 'black':
    ...
    def is_legal_castle(self, current_pos, end_pos, color):
        if color == 'white' and (end_pos[0], int(end_pos[1])) in [('c', 1), ('g', 1)] and \
                (current_pos[0], int(current_pos[1])) == ('e', 1):
            temp = self.is_threat(current_pos, current_pos, color)
            if self.squares[(current_pos[0], int(current_pos[1]))]['castle'] and not temp[0]:
                if end_pos[0] == 'c' and self.squares[('b', 1)]['occupied'] is False:
                    if self.squares[('a', 1)]['castle'] is True:
                        temp2 = True
                        for i in ['c', 'd']:
                            temp3 = self.is_threat(current_pos, (i, 1), 'white')
                            if not (self.squares[(i, 1)]['occupied'] is False and
                                    not temp3[0]):
                                temp2 = False
                        return temp2
                elif end_pos[0] == 'g':
                    if self.squares[('h', 1)]['castle'] is True:
                        temp2 = True
                        for i in ['f', 'g']:
                            temp3 = self.is_threat(current_pos, (i, 1), 'white')
                            if not (self.squares[(i, 1)]['occupied'] is False and
                                ...
                                temp2 = False
                        return temp2
        elif color == 'black' and (end_pos[0], int(end_pos[1])) in [('c', 8), ('g', 8)] and \
                (current_pos[0], int(current_pos[1])) == ('e', 8):
            temp = self.is_threat(current_pos, current_pos, color)
            if self.squares[(current_pos[0], int(current_pos[1]))]['castle'] and not temp[0]:
                if end_pos[0] == 'c' and self.squares[('b', 8)]['occupied'] is False:
                    if self.squares[('a', 8)]['castle'] is True:
                        temp2 = True
                        for i in ['c', 'd']:
                            temp3 = self.is_threat(current_pos, (i, 8), 'white')
                            if not (self.squares[(i, 8)]['occupied'] is False and
                                    not temp3[0]):
                                temp2 = False
                        return temp2
                elif end_pos[0] == 'g':
                    if self.squares[('h', 8)]['castle'] is True:
                        temp2 = True
                        for i in ['f', 'g']:
                            temp3 = self.is_threat(current_pos, (i, 8), 'white')
                            if not (self.squares[(i, 8)]['occupied'] is False and
    ...
    def move_piece(self, color):
        while True:
            while True:
                current_pos = input(
                    'What piece would you (' + str(color) + ') like to move? Type \'help\' for assistance.\nInput:')
                if current_pos == 'help':
                    print('To make a move, type the column letter followed by the row number\n' +
                    ...
                    continue
                if type(current_pos) == str and len(current_pos) == 2 and current_pos[0] in self.columns and \
                        int(current_pos[1]) in self.rows and color == self.squares[(current_pos[0],
                                                                                    int(current_pos[1]))]['player']:
                    print('Input received.')
                    break
                elif True:
                    print('You either typed something that wasn\'t a square, that square is\n' +
                          'unoccupied, or you do not own that piece.')
                    continue
            while True:
                end_pos = input('Where would you like to move your ' +
                                str(self.squares[(current_pos[0], int(current_pos[1]))]['occupied']) +
                                ' (currently at ' + current_pos + ') to?\nInput:')
                if end_pos == 'help':
                    print('To finish making a move, type the column letter followed by the row number \
                        of the square you would like to move the piece to. Example: \'c3\'.\n' +
                          'If you want to select a different piece, type \'back\'.')
                    continue
                elif end_pos == 'back':
                    break
                elif not (type(end_pos) == str and len(end_pos) == 2 and end_pos[0] in self.columns and
                          int(end_pos[1]) in self.rows):
                    print('You typed something that wasn\'t a valid square.')
                    self.visualize()
                    continue
                elif type(end_pos) == str and len(end_pos) == 2 and end_pos[0] in self.columns and \
                        int(end_pos[1]) in self.rows and (self.is_legal(current_pos, end_pos, color) or
                                                          self.is_legal_castle(current_pos, end_pos, color)):
                    break
                elif type(end_pos) == str and len(end_pos) == 2 and end_pos[0] in self.columns and \
                        int(end_pos[1]) in self.rows and not self.is_legal(current_pos, end_pos, color) and \
            ...
            if self.is_legal(current_pos, end_pos, color):
                ...
                if temp[0]:
                    i = temp[1]
                    j = temp[2]
                    print('That move would place white in check from ' + str(self.squares[(j, i)]['occupied']) +
                          ' at ' + str(j) + str(i) + '.')
                elif not temp[0]:
                    cur_col_index = self.columns.index(current_pos[0])
                    end_col_index = self.columns.index(end_pos[0])
                    ...
                    if self.squares[(end_pos[0], int(end_pos[1]))]['occupied'] is not False or \
                            self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'pawn':
                        self.fifty = self.turn_count
                    # Flagging for en passant
                    if self.squares[(current_pos[0], int(current_pos[1]))]['player'] == 'white' and \
                            self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'pawn':
                        if int(current_pos[1]) == 2:
                            if self.squares[(current_pos[0], 3)]['occupied'] is False and \
                                    self.squares[(current_pos[0], 4)]['occupied'] is False and \
                                ...
                                if cur_col_index > 0:
                                    # noinspection PyTypeChecker
                                    self.squares[(self.columns[cur_col_index - 1], 4)]['en passant'] = \
                                        [self.turn_count + 1, self.columns[cur_col_index]]
                                if cur_col_index < 7:
                                    # noinspection PyTypeChecker
                                    self.squares[(self.columns[cur_col_index + 1], 4)]['en passant'] = \
                                        [self.turn_count + 1, self.columns[cur_col_index]]
                        elif self.squares[(end_pos[0], int(end_pos[1]))]['occupied'] is False and \
                                cur_col_index != end_col_index:
                            self.squares[(end_pos[0], int(current_pos[1]))]['occupied'] = False
                            self.squares[(end_pos[0], int(current_pos[1]))]['player'] = False
                    if self.squares[(current_pos[0], int(current_pos[1]))]['player'] == 'black' and \
                            self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'pawn':
                        if int(current_pos[1]) == 7:
                            if self.squares[(current_pos[0], 6)]['occupied'] is False and \
                                    self.squares[(current_pos[0], 5)]['occupied'] is False and \
                                ...
                                if cur_col_index > 0:
                                    # noinspection PyTypeChecker
                                    self.squares[(self.columns[cur_col_index - 1], 5)]['en passant'] = \
                                        [self.turn_count + 1, self.columns[cur_col_index]]
                                if cur_col_index < 7:
                                    # noinspection PyTypeChecker
                                    self.squares[(self.columns[cur_col_index + 1], 5)]['en passant'] = \
                    ...
                    if self.squares[(current_pos[0], int(current_pos[1]))]['occupied'] == 'rook' and \
                            (current_pos[0], int(current_pos[1])) in [('a', 1), ('a', 8), ('h', 1), ('h', 8)]:
                        self.squares[(current_pos[0], int(current_pos[1]))]['castle'] = False
                    self.history[self.turn_count] = (self.squares[(current_pos[0], int(current_pos[1]))]['occupied'],
                    ...
                    self.turn_count += 1
                    if self.squares[(end_pos[0], int(end_pos[1]))]['occupied'] == 'pawn' and int(end_pos[1]) in [1, 8]:
                        while True:
                            promo = input('What piece would you like to promote your pawn to? Type\n'
                            ...
                            if promo not in ['queen', 'rook', 'knight', 'bishop']:
                                ...
                                # noinspection PyTypeChecker
                                self.squares[(end_pos[0], int(end_pos[1]))]['occupied'] = promo
                                break
                    self.visualize()
                    break
                            ...
                            for n in self.columns:
                                if self.squares[(n, m)]['player'] != 'black':
                                    if self.is_legal((j, i), (n, m), 'black'):
                                        if not self.is_threat((j, i), (n, m), 'black')[0] and is_fifty < 101:
                                            return False
                            ...
                            for n in self.columns:
                                if self.squares[(n, m)]['player'] != 'white':
                                    if self.is_legal((j, i), (n, m), 'white'):
                                        if not self.is_threat((j, i), (n, m), 'white')[0] and is_fifty < 101:
                                            return False
            if in_check and is_fifty < 101:
                return 'checkmate'
            elif not in_check and is_fifty < 101:
